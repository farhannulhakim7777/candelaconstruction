...
const btn = form.querySelector('[type="submit"]')
      ...
      setTimeout(() => {
        ...
        const waText = `Halo Candela Construction 👋\n\nNama: ${g(
          'name'
        )}\nPerusahaan: ${g('company')}\nEmail: ${g('email')}\n\nLayanan: ${g(
          'service'
        )}\nAlamat Perusahaan: ${g('alamat')}\n\nPesan:\n${g('message')}`

        const emailSubject = `Enquiry Baru dari ${g('name')} - ${g('service')}`
        const emailBody = `Nama: ${g('name')}\nEmail: ${g('email')}\nLayanan: ${g(
          'service'
        )}\nAlamat Proyek: ${g('alamat')}\n\nPesan:\n${g('message')}`

        // Buka WhatsApp dengan pesan terisi
        window.open(
          'https://wa.me/6281113092828?text=' + encodeURIComponent(waText),
          '_blank'
        )

        // Buka email client dengan pesan terisi
        window.location.href =
          'mailto:candelaconstruction@gmail.com?subject=' +
